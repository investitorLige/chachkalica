...
* ``metrics``     — ``evaluate_detection`` and the by-name class remap
                    (imported by ``pipeline.py``; the remap is what lets several
                    bundled models be merged into one class space)
...
toolkit (model building, dataset loaders, experiment configs) and the two YAML
helpers ``Pipeline.run()`` uses to serialize a batch-eval result, which lives in
``friendy_chachkalica/ml/val.py`` alongside imports of the whole trainer.
...
from ._vendor.metrics import evaluate_detection, remap_raw_predictions_to_eval_classes
