...
Runs on CPU onnxruntime (fp32 execution, fully supported) in seconds. Usage:
    ...
    python -m friendy_chachkalica.ml.trt_export.fp16_overflow_probe --arch rtdetr
"""
...
def _setup(arch):
    ...
    torch.manual_seed(0)
    if arch == "rtdetr":
        from friendy_chachkalica.ml.onnx_export.arch.rtdetr import export_rtdetr
    ...
    if arch == "rfdetr":
        from friendy_chachkalica.ml.onnx_export.arch.rfdetr import export_rfdetr
    ...
    if arch == "fasterrcnn":
        ...
        from friendy_chachkalica.ml.onnx_export.arch.fasterrcnn import export_fasterrcnn
