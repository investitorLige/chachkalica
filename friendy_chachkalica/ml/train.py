...
try:
    from ..preprocess.crop_cache import build_crop_cache
    from ..preprocess.cropping import crop_batch
    from ..preprocess.tiling import tile_batch
except ImportError:
    from preprocess.crop_cache import build_crop_cache
    from preprocess.cropping import crop_batch
    from preprocess.tiling import tile_batch
...
def build_run_pipeline(
    ...
    config: ExperimentConfig,
    device: torch.device,
    box_cache: Optional[Dict[Any, Any]] = None,
):
    ...
    ``box_cache``, when given, is shared across every pipeline this call
    produces (and, by the caller, across every model stage in the run): a
    per-image-id cache of detected person boxes so the frozen detector isn't
    re-run for an image it's already seen (see ``chachak.pipeline.Pipeline``).
    Also used with ``adapter=None`` purely to run the detector for
    :func:`crop_cache.build_crop_cache`'s one-time precompute pass — the
    person-box side of a pipeline never touches ``model_adapter``.
    """
    ...
    if spec.detector_checkpoint is not None:
        ...
        if spec.detector_min_box_size is not None:
            detector_raw["min_box_size"] = spec.detector_min_box_size
    ...
    pipeline_config = pipeline_config_from_dict(raw, Path(__file__).resolve().parent.parent)
    ...
    return build_pipeline(pipeline_config, adapter, device, detector, box_cache=box_cache)
...
def train_experiment(
    ...
    # Person-crop pipelines (people_detect_first/batch_people) run a *frozen*
    # detector, so its output for a given image never changes across epochs or
    # across the model stages this experiment trains (e.g. a rtdetr stage then
    # a yolox stage on the same data). Precompute those crops once instead of
    # re-running the detector every batch of every epoch. ``crop_pipeline``
    # (adapter=None: only its detector/box-finding side is ever exercised)
    # does the precompute pass; ``box_cache`` is then also handed to every
    # stage's own eval_pipeline (see train_model), so val-mAP/test prediction
    # — which must keep working on full frames, see train_model's docstring —
    # skip the detector too after their first hit instead of only train/val-loss.
    box_cache: Dict[Any, Any] = {}
    crop_pipeline = None
    materialized_datasets: Dict[tuple, DatasetConfig] = {}
    crops_materialized = config.pipeline is not None and _pipeline_needs_detector(config.pipeline)
    if crops_materialized:
        print(
            f"[train] Precomputing person crops once for pipeline={config.pipeline.name} "
            "(frozen detector output is reused across every epoch and model stage)"
        )
        crop_pipeline = build_run_pipeline(None, config, device, box_cache=box_cache)
    ...
    try:
        for run in runs:
            ...
            train_dataset_for_loader = _materialize_crops_if_needed(
                config, run.train_dataset, crop_pipeline, device, materialized_datasets
            )
            train_loader = _get_train_loader(config, train_dataset_for_loader, train_loaders)
            ...
            # val_loss_loader only exists (non-None) when crops are actually
            # materialized — train_model treats a None here as "fall back to
            # val_loader with the original live-transform auto-detection",
            # which must stay exactly today's behavior for tiling/no-pipeline
            # experiments (val_loader would otherwise get reused as-is here too,
            # which is correct data but would wrongly flip apply_transform off).
            val_loss_loader = None
            if crops_materialized:
                val_dataset_for_loss = _materialize_crops_if_needed(
                    config, run.val_dataset, crop_pipeline, device, materialized_datasets
                )
                val_loss_loader = _get_eval_loader(config, val_dataset_for_loss, eval_loaders)
            ...
            try:
                result = train_model(
                    ...
                    box_cache=box_cache,
                    crops_materialized=crops_materialized,
                )
        ...
        # Only a training-time optimization, never a deliverable artifact —
        # clear it once the run is done so it doesn't linger on disk. A
        # graceful "Kill run gracefully" already removes it for free (it's
        # nested under output_dir, which that action rmtree's wholesale); this
        # covers the normal-completion / early-stop / raised-error paths that
        # don't go through that teardown.
        if crops_materialized:
            cache_dir = config.output_dir / _PERSON_CROPS_CACHE_DIRNAME
            shutil.rmtree(cache_dir, ignore_errors=True)
            print(f"[train] Cleared precomputed person-crop cache: {cache_dir}")
...
# Person-crop pipelines get their crops precomputed once per run into this
# subdirectory of the run's output_dir (see _materialize_crops_if_needed).
# Living inside output_dir means the existing whole-output_dir teardown
# ("Kill run gracefully" -> cleanup.remove_run_artifacts) already force-clears
# it for free; train_experiment additionally clears it on normal completion
# (see the try/finally around the run loop below).
_PERSON_CROPS_CACHE_DIRNAME = "_person_crops_cache"


def _materialize_crops_if_needed(
    config: ExperimentConfig,
    dataset_config: Optional[DatasetConfig],
    crop_pipeline: Any,
    device: torch.device,
    materialized: Dict[tuple, DatasetConfig],
) -> Optional[DatasetConfig]:
    """Swap in a precomputed-crops ``DatasetConfig`` for a loss-only consumer.

    No-op (returns ``dataset_config`` unchanged) when there's no dataset or no
    person-crop pipeline in play. Memoized on the *original* dataset's cache
    key, so datasets shared across model stages (the common case: one
    experiment training several architectures on the same data) only get
    cropped once per run, not once per stage.
    """
    if dataset_config is None or crop_pipeline is None:
    ...
    if cached is not None:
        return cached
    cache_dir = (
        config.output_dir / _PERSON_CROPS_CACHE_DIRNAME / dataset_config.role / dataset_config.name
    )
    result = build_crop_cache(dataset_config, crop_pipeline, device, cache_dir)
...
def train_model(
    ...
    evaluate_after_train: bool = True,
    resume: bool = False,
    val_loss_loader: Optional[DataLoader] = None,
    box_cache: Optional[Dict[Any, Any]] = None,
    crops_materialized: bool = False,
) -> Dict[str, Any]:
    ...
    eval_pipeline = build_run_pipeline(adapter, config, device, box_cache=box_cache)
    if eval_pipeline is not None:
        ...
        if config.pipeline.name in _TRAINABLE_PIPELINES:
            if _pipeline_needs_detector(config.pipeline):
                if crops_materialized:
                    print(
                        "[train] Training on precomputed person crops: the person "
                        "detector ran once for this run (not per epoch); "
                        "ground-truth objects outside every detected person crop "
                        "are still not seen (detector recall bounds train too)."
                    )
                else:
                    print(
    ...
    for epoch in range(start_epoch, config.training.epochs + 1):
        ...
        train_summary = train_one_epoch(
            ...
            apply_transform=(False if crops_materialized else None),
        )
        ...
        if run_val:
            ...
            # evaluate_loss only ever needs crop images + crop-local targets (no
            # merge back to full-frame coordinates), so it can read the
            # precomputed crop cache when there is one; evaluate_map below
            # cannot (it needs full frames to merge per-crop predictions back
            # for scoring) and always uses the original val_loader.
            loss_loader = val_loss_loader if val_loss_loader is not None else val_loader
            val_summary = evaluate_loss(
                ...
                pipeline=eval_pipeline,
                apply_transform=(False if val_loss_loader is not None else None),
            )
...
def train_one_epoch(
    ...
    # ``apply_transform`` lets a caller override the auto-detected default —
    # used when ``loader`` already yields precomputed person crops (see
    # crop_cache.build_crop_cache), so this loop doesn't crop them a second
    # time.
