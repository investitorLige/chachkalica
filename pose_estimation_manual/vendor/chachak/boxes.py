...
from .formats import clip_xyxy, xyxy_to_xywhn
...
def upscale_crop_to_min_size(
    crop: torch.Tensor, min_size: int
) -> Tuple[torch.Tensor, Tuple[int, int]]:
    """Bilinearly upscale a CHW crop's short side(s) up to ``min_size``.
...
PRED_WIDTH = 6
"""Columns in a Friendy prediction row: ``cx, cy, w, h, confidence, class_id``.

Anything past this is opaque per-detection payload that the geometry helpers
below carry through without interpreting.
"""


def _pred_width(preds: torch.Tensor) -> int:
    """The column count to reshape an empty/degenerate ``preds`` back to."""
    if preds.ndim >= 2 and preds.shape[-1] >= PRED_WIDTH:
...
def remap_local_preds_to_frame(
    ...
    pixel offset ``offset_xy``. Returns the same width normalized to the full
    frame, carrying confidence and class_id unchanged.

    Columns beyond the first six are opaque payload, carried through untouched —
    only the four box coordinates mean anything geometrically, so a caller can
    ride extra per-detection data along through the remap without this function
    needing to know what it is (``pipeline._crop_infer_remap`` sends the index of
    the person crop each row came out of this way).
    """
    if preds.numel() == 0:
        return preds.reshape(-1, _pred_width(preds))
    ...
    return torch.cat([xywhn, preds[:, 4:]], dim=1)
...
def merge_predictions(
    ...
    :func:`remap_local_preds_to_frame`) and must agree on their width.
    ``min_box_size`` drops boxes whose pixel width or height is below the
    threshold. Returns a single ``(M, 6+)`` tensor of that same width: the NMS
    keeps whole rows, so any opaque trailing payload survives on the survivors.
    """
    preds = [p for p in preds_list if p is not None and p.numel() > 0]
    if not preds:
        empty_width = max((_pred_width(p) for p in preds_list if p is not None),
                          default=PRED_WIDTH)
        return torch.zeros((0, empty_width))
    width = _pred_width(preds[0])
