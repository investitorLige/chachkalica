...
downstream of it (``Detection.boxes``, danger zones, the admin) knows what to
do with one — the same gap :mod:`.scrfd` hit with its 5-point landmarks. But
where SCRFD's landmarks are simply dropped, here the keypoints are the reason
this bundle exists: this handler spends them on a **posture classification**
(standing / sitting / lying), computed once per person from joint geometry,
and emits *that* as Contract A's ``labels`` — three classes rather than the
one raw keypoints could never carry anyway. The raw keypoints themselves are
still discarded afterward for the same reason SCRFD drops landmarks: there is
nowhere further downstream to put them.
...
surgery needed (confirmed by building this arch's engine directly from the
unmodified export) — so, unlike SCRFD, this handler runs no suppression of
its own. The two outputs it receives are already the NMS'd, index-matched
pair.
"""
